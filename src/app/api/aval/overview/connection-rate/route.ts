...
import { getAvalLeads, getAvalConnectedCalls } from "@/lib/airtable/tables-aval";
...
/**
 * First Eastern day the "paid" / "organic" GHL tags were being applied to new
...
export async function GET(request: NextRequest) {
  ...
  const [leads, connected] = await Promise.all([getAvalLeads(), getAvalConnectedCalls()]);
